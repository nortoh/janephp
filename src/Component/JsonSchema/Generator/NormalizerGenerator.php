...
class NormalizerGenerator implements GeneratorInterface
{
    ...
    public function generate(Schema $schema, string $className, Context $context): void
    {
        ...
        foreach ($schema->getClasses() as $class) {
            ...
            $useStmts = [
                new Stmt\Use_([new Node\UseItem(new Name('Jane\Component\JsonSchemaRuntime\Reference'))]),
                new Stmt\Use_([new Node\UseItem(new Name($this->naming->getRuntimeClassFQCN($schema->getNamespace(), ['Normalizer'], 'CheckArray')))]),
                new Stmt\Use_([new Node\UseItem(new Name($this->naming->getRuntimeClassFQCN($schema->getNamespace(), ['Normalizer'], 'ValidatorTrait')))]),
                new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\Serializer\Exception\InvalidArgumentException'))]),
                new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\Serializer\Normalizer\DenormalizerAwareInterface'))]),
                new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\Serializer\Normalizer\DenormalizerAwareTrait'))]),
                new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\Serializer\Normalizer\DenormalizerInterface'))]),
                new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\Serializer\Normalizer\NormalizerAwareInterface'))]),
                new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\Serializer\Normalizer\NormalizerAwareTrait'))]),
                new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\Serializer\Normalizer\NormalizerInterface'))]),
                new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\HttpKernel\Kernel'))]),
            ];

            if ($this->useCacheableSupportsMethod) {
                $useStmts[] = new Stmt\Use_([new Node\UseItem(new Name('Symfony\Component\Serializer\Normalizer\CacheableSupportsMethodInterface'))]);
            }
    ...
    private function symfony7support(array $symfony7Stmts, array $legacyStmts): array
    {
        return [new Stmt\If_(
            new Expr\BinaryOp\LogicalOr(
                ...
                new Expr\BinaryOp\LogicalOr(
                    new Expr\BinaryOp\GreaterOrEqual(new Expr\ClassConstFetch(new Name('Kernel'), new Identifier('MAJOR_VERSION')), new Scalar\Int_(7)),
                    new Expr\BinaryOp\LogicalAnd(
                        new Expr\BinaryOp\Identical(new Expr\ClassConstFetch(new Name('Kernel'), new Identifier('MAJOR_VERSION')), new Scalar\Int_(6)),
                        new Expr\BinaryOp\Identical(new Expr\ClassConstFetch(new Name('Kernel'), new Identifier('MINOR_VERSION')), new Scalar\Int_(4)),
                    )
                ),
            ),
            [

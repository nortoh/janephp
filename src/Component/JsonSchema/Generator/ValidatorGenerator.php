            ...
            if ($class->hasValidatorGuesses()) {
                ...
                foreach ($class->getPropertyValidatorGuesses() as $name => $propertyGuesses) {
                    ...
                    foreach ($propertyGuesses as $propertyGuess) {
                        $constraints[] = new ArrayItem($this->generateConstraint($propertyGuess));
                    }
                ...
                foreach ($class->getValidatorGuesses() as $classGuess) {
                    if ($classGuess->getSubProperty() === null) {
                        $constraintsItems[] = new ArrayItem($this->generateConstraint($classGuess));
                    } else {
                ...
                foreach ($collectionItemsConstraints as $name => $constraints) {
                    ...
                    $collectionItems[] = new ArrayItem(new Expr\New_(new Node\Name\FullyQualified($collectionClass), [
                ...
                if (\count($collectionItems) > 0) {
                    ...
                    $constraintsItems[] = new ArrayItem(new Expr\New_(new Node\Name\FullyQualified(Collection::class), [
                        new Node\Arg(new Expr\Array_([
                            new ArrayItem(new Expr\Array_($collectionItems), new Scalar\String_('fields')),
                            new ArrayItem(new Expr\ConstFetch(new Node\Name($allowExtraFields)), new Scalar\String_('allowExtraFields')),
                        ])),
                    ]));
                }
        ...
        foreach ($guess->getArguments() as $argName => $argument) {
            ...
            if (\is_array($argument)) {
                ...
                foreach ($argument as $item) {
                    $values[] = new ArrayItem(new Scalar\String_($item));
                }
            ...
            if (null !== $value) {
                $args[] = new ArrayItem($value, new Scalar\String_($argName));
            }
        }
